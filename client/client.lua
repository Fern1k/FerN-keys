...
function LockSystem(playerPed, vehicle)
    ...
    if vehicle and vehicle ~= 0 then
        ...
        ESX.TriggerServerCallback('ls:check', function(hasKey, isOwned, isLocked)
            ...
            elseif isLocked or isOwned then
                ...
                LockTask = false
            elseif not TriedPlates[plate] then
    		TriedPlates[plate] = true -- Zaznacz, że już próbowałeś na tym pojeździe

   		 if GetRandomIntInRange(1, 100) < Config.percentage then
    ...
    ESX.ShowNotification("Już próbowałeś znaleźć klucze w tym pojeździe!")
    LockTask = false
end
        end, plate)
    else
...
function useKey(plate)
    ...
    for _, veh in pairs(vehs) do
        ...
        if type(platet) == "string" then
            platet = platet:gsub("%s+", ""):lower()
        end
...
exports("removeKeys", function(plate)
    TriggerServerEvent("ls:removeKeysFromPlayer", plate)
end)